...
.color-dot{width:20px;height:20px;border-radius:50%;border:2px solid rgba(255,255,255,0.3)}
    ...
    const createControllerIcon = () => {
        ...
        const path = document.createElementNS(xmlns, 'path');
        path.setAttribute('d', 'M6 11h4v2H6zm10 0a2 2 0 012 2 2 2 0 01-2 2 2 2 0 01-2-2 2 2 0 012-2M6 7h4v2H6m12 2a9 9 0 00-9-9 9 9 0 00-9 9 9 9 0 003.46 7.13c.85.63 1.89.87 2.54.87.68 0 1.56-.27 1.88-.59L9 15.19V15l-1-4h8l-1 4v.19l1.12 1.22c.32.32 1.2.59 1.88.59.65 0 1.69-.24 2.54-.87A9 9 0 0021 9z');
        svg.appendChild(path);
        
        return svg;
    };

    const createDiscordIcon = () => {
        const xmlns = 'http://www.w3.org/2000/svg';
        const svg = document.createElementNS(xmlns, 'svg');
        svg.setAttribute('class', 'w-6 h-6 inline-block');
        svg.setAttribute('fill', 'currentColor');
    ...
    const createMenuButton = (id, iconDs, text, isLink = false, href = '') => {
        ...
        // Special handling for controller and discord icons
        if (id === 'games-menu') {
            const svg = createControllerIcon();
            const span = document.createElement('span');
            span.style.cssText = 'font-size: 10px; display: block; margin-top: -4px;';
            span.textContent = text;
            btn.appendChild(svg);
            btn.appendChild(span);
        } else if (id === 'discord-link') {
            const svg = createDiscordIcon();
            const span = document.createElement('span');
            span.style.cssText = 'font-size: 10px; display: block; margin-top: -4px;';
            span.textContent = text;
            btn.appendChild(svg);
    ...
    const gamesMenu = createMenuButton('games-menu', 'M11 3.055A9.001 9.001 0 1020.945 13H11V3.055zM20.488 9H15V3.512A9.025 9.025 0 0120.488 9z', 'Games');
    ...
    addMessage("Hello! I'm Noobs Gui. Ask me a question to get started. Check out the Settings menu to change colors!", 'ai');
...
<div class="card">
    ...
    document.addEventListener('mouseup', () => {
        if (isDragging) {
            widgetContainer.style.transition = 'all 0.3s';
        }
        isDragging = false;
